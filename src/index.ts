...
import { search, getSearchSuggestions, analyzeQuery, editDistance, isFuzzyMatch } from "./engine/searcher";
import { SearchOptions, CacheProvider, IndexStorage, Posting } from "./types";
...
export class SmartSearch {
   ...
   * ```
   */
  async index(doc: any, collection: string, lang: "en" | "ar" = "en"): Promise<void> {
   ...
   * ```
   */
  async search(q: string, options: SearchOptions): Promise<any[]> {
   ...
   * ```
   */
  async getSuggestions(partialQuery: string, options: SearchOptions, limit: number = 5): Promise<string[]> {
    return getSearchSuggestions(partialQuery, this.mongo, options, limit);
  }
   ...
   */
  analyzeQuery(query: string, language: "en" | "ar" = "en"): any {
